...
REM Deploy to AWS ECS Fargate Script for BankManagementSystem
...
REM Handle load balancer creation if needed
...
powershell -Command "(Get-Content '!TASK_DEF_TEMP!') -replace '{{IMAGE_URI}}', '!IMAGE_URI!' -replace '{{AWS_REGION}}', '!AWS_REGION!' -replace '{{ACCOUNT_ID}}', '!ACCOUNT_ID!' -replace '{{DB_CONNECTION_STRING}}', '!DB_CONNECTION_STRING!' | Set-Content '!TASK_DEF_TEMP!'"
...
REM Replace placeholders in service definition using PowerShell
